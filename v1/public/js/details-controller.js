...
(function (window, $) {
  ...
  $.extend(window.DetailsController.prototype, {
    ...
    insertHTML: function (subject) {
      const $subjectId = this.$wrapper.find('.js-subject-id')
      const $burger = this.$wrapper.find('.js-burger')
    ...
    manageExpandable: function (e) {
      const $svg = this.$element.find('.expandable .trigger .property svg')
      const $body = this.$element.find('.expandable .body')
    ...
    edit: function (subject) {
      ...
      $.ajax({
        ...
        success: function (data) {
          executeAfter(function () {
            const $handle = controller.popupManager.display(data)
            if (!$handle) {
              return
            }
            $handle.find('form').on(
